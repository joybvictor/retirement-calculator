#!/usr/bin/env python3
"""
Comprehensive Retirement Calculator - Phase 1
Web-based retirement planning with 401k, IRAs, taxable accounts, expenses, home value, 
taxes, Medicare, inflation, RMDs, and year-by-year breakdown to age 80
"""
...
def calculate_retirement_expenses(current_monthly_expenses: float, 
    ...
    # Apply retirement expense adjustment (typically 80% of current)
...
def generate_amortization_schedule(starting_401k: float, starting_trad_ira: float,
    ...
    """Generate year-by-year amortization schedule for all accounts to age 80"""
    schedule = []
    ...
    # Calculate initial withdrawal amounts
    total_taxable_accounts = starting_401k + starting_trad_ira
    annual_withdrawal_taxable = total_taxable_accounts * withdrawal_percent
    annual_withdrawal_roth = starting_roth_ira * withdrawal_percent
    annual_withdrawal_taxable_acct = starting_taxable * withdrawal_percent
    ...
    for year in range(1, years + 1):
        if balance_401k + balance_trad_ira + balance_roth_ira + balance_taxable <= 0:
            break
            
        current_age = retirement_age + year - 1
        
        # Beginning balances
        begin_401k = balance_401k
        begin_trad_ira = balance_trad_ira
        begin_roth_ira = balance_roth_ira
        begin_taxable = balance_taxable
        total_begin = begin_401k + begin_trad_ira + begin_roth_ira + begin_taxable
        
        # Calculate investment growth for each account
        ...
        # Calculate RMDs for tax-deferred accounts (age 73+)
        rmd_401k = calculate_rmd(balance_401k, current_age)
        rmd_trad_ira = calculate_rmd(balance_trad_ira, current_age)
        total_rmd = rmd_401k + rmd_trad_ira
        
        # Determine actual withdrawals
        total_taxable_balance = balance_401k + balance_trad_ira
        if total_taxable_balance > 0:
            pct_401k = balance_401k / total_taxable_balance
            pct_trad_ira = balance_trad_ira / total_taxable_balance
            
            planned_401k = annual_withdrawal_taxable * pct_401k
            planned_trad_ira = annual_withdrawal_taxable * pct_trad_ira
            
            withdrawal_401k = max(planned_401k, rmd_401k)
            withdrawal_trad_ira = max(planned_trad_ira, rmd_trad_ira)
        else:
            withdrawal_401k = 0
            withdrawal_trad_ira = 0
        
        # Roth and taxable withdrawals
        withdrawal_roth = min(annual_withdrawal_roth, balance_roth_ira + growth_roth_ira)
        withdrawal_taxable_acct = min(annual_withdrawal_taxable_acct, balance_taxable + growth_taxable)
        ...
        medicare_costs = 0
        if include_medicare:
            medicare_costs = calculate_medicare_costs(current_age, gross_income)
        
        # Calculate taxes
        taxes = calculate_taxes(
            withdrawal_401k, withdrawal_trad_ira, withdrawal_taxable_acct,
            adjusted_pension / 12, adjusted_ss / 12,
        ...
        schedule.append({
            ...
            '401k Start': begin_401k,
            'Trad IRA Start': begin_trad_ira,
            'Roth IRA Start': begin_roth_ira,
            'Taxable Start': begin_taxable,
            'Total Growth': total_growth,
            '401k Growth': growth_401k,
            'Trad IRA Growth': growth_trad_ira,
            'Roth Growth': growth_roth_ira,
            'Taxable Growth': growth_taxable,
            '401k Withdrawal': withdrawal_401k,
            'Trad IRA Withdrawal': withdrawal_trad_ira,
            'Roth Withdrawal': withdrawal_roth,
            'Taxable Withdrawal': withdrawal_taxable_acct,
            'Total Withdrawal': total_withdrawal,
            'RMD Required': 'Yes' if total_rmd > 0 else 'No',
            ...
            'Total End': total_end,
            '401k End': end_401k,
            'Trad IRA End': end_trad_ira,
            'Roth IRA End': end_roth_ira,
            'Taxable End': end_taxable
        })
        ...
        annual_withdrawal_taxable *= (1 + inflation)
        annual_withdrawal_roth *= (1 + inflation)
        annual_withdrawal_taxable_acct *= (1 + inflation)
...
def calculate_projection(current_age: int, 
    ...
    taxes = calculate_taxes(
        annual_taxable_withdrawal * 0.6,
        annual_taxable_withdrawal * 0.4,
    ...
    amortization = generate_amortization_schedule(
        ...
        4.0, return_rate, adjusted_pension, adjusted_ss,
...
def main():
    ...
    with st.sidebar:
        ...
        current_monthly_expenses = st.number_input(
            ...
            help="Your current monthly spending (housing, food, utilities, etc.)"
        )
        ...
        retirement_expense_pct = st.slider(
            ...
            help="Typically 70-90% of pre-retirement expenses"
        )
    ...
    for proj in projections:
        ...
        if current_home_value > 0:
            row['Home Value'] = format_currency(proj['projected_home_value'])
            row['Complete Net Worth'] = format_currency(proj['total_net_worth'])
        
        row['Net Monthly Income'] = format_currency(proj['net_monthly_income'])
        
        if current_monthly_expenses > 0:
            row['Monthly Expenses'] = format_currency(proj['monthly_retirement_expenses'])
    ...
    for tab, proj in zip(tabs, projections):
        with tab:
            ...
            if current_home_value > 0:
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("🏠 Home Value at Retirement", format_currency(proj['projected_home_value']))
                with col2:
                    st.metric("💎 **Complete Net Worth**", format_currency(proj['total_net_worth']))
            ...
            if current_monthly_expenses > 0:
                ...
                with col2:
                    ...
                    st.metric("Annual Expenses", format_currency(proj['annual_expenses']))
                
                with col3:
                    surplus_delta = proj['monthly_surplus_shortfall']
                    coverage = proj['expense_coverage_ratio']
                    
                    if surplus_delta >= 0:
                        st.metric("Monthly Surplus", format_currency(surplus_delta), 
                                 delta="✅ Covered", delta_color="normal")
                        st.metric("Coverage Ratio", f"{coverage:.0f}%", 
                                 delta="✅ Good" if coverage >= 100 else "⚠️ Low", 
                                 delta_color="normal" if coverage >= 100 else "inverse")
                    else:
                        st.metric("Monthly Shortfall", format_currency(abs(surplus_delta)), 
            ...
            if proj['amortization']:
                ...
                # Summary statistics
                total_growth = amort_df['Total Growth'].sum()
                total_withdrawals = amort_df['Total Withdrawal'].sum()
                final_balance = amort_df.iloc[-1]['Total End']
                final_age = amort_df.iloc[-1]['Age']
                
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total Growth", format_currency(total_growth))
                with col2:
                    st.metric("Total Withdrawals", format_currency(total_withdrawals))
                with col3:
                    st.metric(f"Balance at {final_age}", format_currency(final_balance))
                with col4:
                    if final_balance > 10000:
                        st.metric("Status", "✅ Funded", delta="Past age 80")
                    else:
                        st.metric("Status", "⚠️ Low", delta="Review plan")
                
                # Display table
                display_columns = ['Year', 'Age', 'Total Start', 'Total Growth', 'Total Withdrawal', 
                                  'RMD Required', 'Net Income', 'Annual Expenses', 'Surplus/Shortfall', 'Total End']
                
                display_df = amort_df[display_columns].copy()
                
                # Format currency columns
                currency_cols = ['Total Start', 'Total Growth', 'Total Withdrawal', 'Net Income', 'Annual Expenses', 'Surplus/Shortfall', 'Total End']
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"retirement_schedule_age_{proj['age']}.csv",
                    mime="text/csv"
                )
    ...
    with st.expander("Click to view important information"):
        ...
        **4% Withdrawal Rule:**
        ...
        - Applied to all retirement accounts
        
        **Expense Planning:**
        - Current expenses are inflation-adjusted to retirement age
        - Typical retirement expenses: 70-90% of pre-retirement spending
        - Consider healthcare, travel, and lifestyle changes
        
        **Home Value:**
        - Appreciates at inflation rate (conservative estimate)
        - Not included in liquid retirement assets
        - Can be accessed through downsizing or reverse mortgage if needed
        
        **Year-by-Year Breakdown:**
        - Shows detailed cash flow from retirement to age 80
        - Includes all income sources and expenses
        - Accounts for RMDs, taxes, Medicare, and inflation
        - Helps identify potential shortfalls early
